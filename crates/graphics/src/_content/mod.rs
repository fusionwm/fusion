...
impl ContentManager {
    ...
    pub fn get_resource<R: Resource>(&self, id: ResourceID) -> Option<&R> {
        let type_id = TypeId::of::<R>();
        self.storages
            .get(&type_id)
            .and_then(|s| s.as_any().downcast_ref::<TypedResourceStorage<R>>())
            .and_then(|s| s.get_resource(id))
    }

    pub fn get_mut_resource<R: Resource>(&mut self, id: ResourceID) -> Option<&mut R> {
        let type_id = TypeId::of::<R>();
        self.storages
            .get_mut(&type_id)
            .and_then(|s| s.as_any_mut().downcast_mut::<TypedResourceStorage<R>>())
            .and_then(|s| s.get_mut_resource(id))
    }
}
